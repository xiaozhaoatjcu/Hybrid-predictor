  function [result,opl,stego_img,PSNR,x_Edge] = embed_half(img,pred_img,pl,method,side,ds,dm,sect_switch,auxiliary)
...
xpl = pl(1,1:uint32(numel(pl)/2));                               %在X集合中嵌入一半隐秘信息
xpl=[xpl 1 0 1];
[x_result] = embed_in_one_set(img, pred_img, xpl, 0, method,side,ds,dm,sect_switch);
if ~x_result.ok
    stego_img = zeros(r,c);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%处理边缘信息
x_result.Edge=[x_result.PZ_infor x_result.side_info  x_result.LB];%%X系列的边缘信息
%%其中PZ_infor是PZ选点的边缘信息

Edge_finite1=(side^2*4+(c-side*2)*side*2+(r-side*2)*side*2)/2;%%边缘信息的上限
Edge_finite2=numel(x_result.Edge)+auxiliary;
if Edge_finite2<=Edge_finite1

    Edge_finite=Edge_finite2;
    O_LSB=zeros(1,Edge_finite);
    tmpdata=1;
    for tmp_r=1:r
         for tmp_c=1:c
             ...
             %%奇数为O系列
             O_LSB(tmpdata)=bitget(x_result.stego_img(tmp_r,tmp_c),1);
             tmpdata=tmpdata+1;
             else
             end
             if tmpdata>Edge_finite
             break
             end
         end
         if tmpdata>Edge_finite
    ...
    opl=[x_result.Edge O_LSB pl(1,uint32(numel(pl)/2)+1:end)];
    %%%%%%
    x_result.dist = sum(sum((double(x_result.stego_img) - double(img)).^2));   %X集合失真

    stego_img = uint8(x_result.stego_img);
...
elseif ismember(method,[1 2 3])
    [surS,cmt,sur]=single_feature(img,pred_errors,r,c,set,method,side);  % sur和surS是对原图计算得出的复杂度
    originw=[];
    featurenumber=1;
    w=1;
    ms=1;
...
%排序
[~,pos] = sort(sur);  % 对复杂度进行从小到大排序，pos为原位置坐标
sortsurS=zeros(ms+2,numel(sur));
